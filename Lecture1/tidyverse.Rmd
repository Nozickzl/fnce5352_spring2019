...
```{r}
who %>%
  gather(group, cases, -country, -iso2, -iso3, -year, -iso_numeric, -g_whoregion)
...
```{r}
who %>%
  select(-iso2, -iso3, -iso_numeric, -g_whoregion) %>%
...
who.summary <- who %>%
  select(-iso2) %>%
  gather(group, cases, -country, -iso3,-iso_numeric, -g_whoregion, -year) %>%
  filter(!is.na(cases)) %>%
  count(year, wt = cases)
